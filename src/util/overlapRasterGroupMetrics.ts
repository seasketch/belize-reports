...
// ADDITIONS: Adding support for raster overlap, only type changes to features
 ...
 */
// ADDITIONS: Adding support for raster overlap, only type changes to featuresByClass
...
const getReducedGroupAreaOverlap = async (options: {
  ...
  const otherOverlap = groupSketches
    .map((groupSketch) =>
      clip(
        genFeatureCollection([groupSketch, ...higherGroupSketches]),
        "difference"
      )
    )
  ...
  const otherRemSketches = genSampleSketchCollection(
    genFeatureCollection<Polygon>(
      flatten(genFeatureCollection(otherOverlap)).features
    )
  ).features;

  const finalFC = genFeatureCollection(
